...
レジストレーション または Csource レジストレーションは、どのデバイスがどのデータをブローカーにフィードするかを示すために使用されます。エンティティ Id とそのタイプ、プロパティとリレーションシップ、プロバイダーのエンドポイント、データの場所などのデータの説明は、Csource リクエストで提供されます。


レジストレーションを作成 (Create registrations)
------------------------------------------------

Csource registration は、次の2つの方法で FogFlow Broker で作成できます:

* Link ヘッダーでのコンテキストの使用: リクエスト ペイロードを解決するためのコンテキストは、リクエストの Link ヘッダーに含まれています。
* ペイロードでのコンテキストの使用: コンテキストはペイロード自体で指定されます。
* Csource registration を作成すると、FogFlow Broker は "201 Created" 応答を返しますが、リクエスト ペイロードにすでにレジスターされているエンティティが少なくとも1つある場合は、"409 Conflict" エラーを返します。

curl リクエストは次のセクションで提供されます。

**Link ヘッダーでのコンテキストの使用:**
...
レジストレーションを更新 (Update registrations)
------------------------------------------------

既存の Csource registration は、その Id でアップデートできます。ペイロードを解決するためのコンテキストは、リクエスト ペイロードで指定されます。コンテキスト オブジェクトがリクエスト ペイロードで指定されていない場合、FogFlow Broker はデフォルトのコンテキストを使用してペイロードを解決します。FogFlow Broker でのレジストレーションのアップデートが成功すると、"204 NoContent" 応答が返されます。

Csource registration のアップデートの curl リクエストを以下に示します。
...
レジストレーションを取得 (Get registrations)
---------------------------------------------

Csource registrations は、クエリ パラメーターを介してリクエストに渡される次のフィルターを使用して FogFlow Broker から取得できます。

* エンティティ タイプに基づく: 一致するエンティティ タイプを持つすべての登録を返します。
* Link ヘッダー付きのエンティティ タイプに基づく: エンティティ タイプが一致するすべてのレジストレーションを返しますが、ここでは、エンティティ タイプの受け渡しが異なります。
* エンティティ Id とエンティティ タイプに基づく: 要求されたエンティティ Id とタイプを含むレジストレーションを返します。
* エンティティ IdPattern およびエンティティ タイプに基づく: 要求されたエンティティ IdPattern の範囲内にあり、エンティティ タイプにも一致するすべてのレジストレーションを返します。

正常に取得すると "200 OK" 応答が返されますが、レジストレーションが存在しない場合、ブローカーは "404 NotFound" エラーを返します。次の curl リクエストをブローカーに送信して、その動作を確認します。
...
レジストレーションを削除 (Delete registrations)
-----------------------------------------------

Csource registration は、次のリクエストを使用して削除できます。
