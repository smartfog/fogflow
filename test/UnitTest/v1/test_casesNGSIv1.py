...
def test_getSubscription18():
    ...
    url = brokerIp + "/ngsi10/subscribeContext"
    headers = {'Content-Type': 'application/json',
               'User-Agent': 'lightweight-iot-broker'}
    r = requests.post(
        url,
        data=json.dumps(
            data_ngsi10.subdata25),
        headers=headers)
    resp_content = r.content
    resInJson = resp_content.decode('utf8').replace("'", '"')
    resp = json.loads(resInJson)
...
def test_case33():
    ...
    headers = {
        'Content-Type': 'appliction/json',
        'fiware-service': 'Abc',
        'fiware-servicepath': 'pqr'}
    r = requests.post(
        url,
        data=json.dumps(
            data_ngsi10.subdata53),
        headers=headers)
    # print(r.content)
