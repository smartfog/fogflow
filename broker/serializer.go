...
func (sz Serializer) SerializeRegistration(expanded []interface{}) CSourceRegistration {
        fmt.Println("Inside SerializeEntity.......")		
            ...
            for k, v := range stringsMap {
                ...
                switch k {
					...
					case DESCRIPTION:					
						...
						registration.Description = sz.getValue(v.([]interface{}))
						break
					case ENDPOINT:					
						fmt.Println("case ENDPOINT.......")
						registration.Endpoint = sz.getValue(v.([]interface{}))
						break
					case EXPIRES:					
					...
					case NAME:					
						...
						registration.Name = sz.getValue(v.([]interface{}))
						break
					default:					
...
func (sz Serializer) getProperty(propertyName string, propertyMap map[string]interface{}) Property {
        ...
        Property.ObservedAt = sz.getObservedAt(propertyMap[OBSERVED_AT].([]interface{}))
...
func (sz Serializer) getRelationship(relationshipName string, relationshipMap map[string]interface{}) Relationship {
        ...
        Relationship.ObservedAt = sz.getObservedAt(relationshipMap[OBSERVED_AT].([]interface{}))
...
func (sz Serializer) getObservedAt(observedAt []interface{}) string {
        var ObservedAt string
        if len(observedAt) > 0 {
                observedAtMap := observedAt[0].(map[string]interface{})
                if observedAtMap[TYPE] == DATE_TIME {
                        ObservedAt = observedAtMap[VALUE].(string)
                }
        }
        return ObservedAt
}
...
func (sz Serializer) getProvidedBy(providedBy []interface{}) ProvidedBy {
        ...
        if len(providedBy) > 0 {
                ...
                ProvidedBy.Object = sz.getObject(providedByMap[HAS_OBJECT])
        }
        return ProvidedBy
}
...
func (sz Serializer) getTimestamp(timestamp []interface{}) <return-type> {
	
}

func (sz Serializer) getInformation(information []interface{}) []RegistrationInfo {
	var regInfoArray []RegistrationInfo{}
	for _,val := range information {
		...
		for k,v := range infoVal {			
			switch k {
				case PROPERTIES:					 
					regInfo.Properties = append(regInfo.Properties, sz.getArrayOfIds(v))
					break
				case RELATIONSHIPS:
					regInfo.Relationships = append(regInfo.Relationships, sz.getArrayOfIds(v))
					break
				case ENTITIES:
					regInfo.Entities = append(regInfo.Entities, sz.getEntities(v))
					break
			}
		}
