...
func (tb *ThinBroker) LDCreateEntity(w rest.ResponseWriter, r *rest.Request) {
        ...
        if ctype, accept := r.Header.Get("Content-Type"), r.Header.Get("Accept"); ctype == "application/json" && accept == "application/ld+json" {
                ...
                resolved := tb.ExpandPayload(w, r, context)
                ...
                serializedEntity := Serializer.SerializeEntity(resolved)
...
func (tb *ThinBroker) ExpandPayload(w rest.ResponseWriter, r *rest.Request, context []interface{}) []interface{} {

        context = append(context, DEFAULT_CONTEXT)

        //get map[string]interface{} of reqBody
        itemsMap := tb.getStringInterfaceMap(w, r)
...
func (tb *ThinBroker) RegisterCSource(w rest.ResponseWriter, r *rest.Request) {
        ...
        if ctype, accept := r.Header.Get("Content-Type"), r.Header.Get("Accept"); ctype == "application/json" && accept == "application/ld+json" {
				...
				//Deserialize payload
				serializedRegistration := Serializer.SerializeRegistration(resolved)
                ...
                RegisterCSourceResp, err := client.RegisterCSource(&serializedRegistration, tb.MyURLPrefix)
                ...
                if err != nil {
                        ...
                        DEBUG.Println("Reg Response: ", string(RegisterCSourceResp))
                        //      w.WriteHeader(201)
...
func (tb *ThinBroker) LDCreateSubscription(w rest.ResponseWriter, r *rest.Request) {
        ...
        var expandedMap []interface{}

        // Get an []interface object
        expanded := tb.ExpandPayload(w, r)

        // Pretty print
        sub, _ := json.MarshalIndent(expanded, "", " ")
        DEBUG.Println("Subscription:")
        DEBUG.Println(string(sub))

        for _, v := range expanded {
                DEBUG.Println("Inside for loop....")
                mp := v.(map[string]interface{})
                DEBUG.Println("Map inside for loop: \n", mp)
                DEBUG.Println("sid from Map: \n", mp["@id"])

                //Create subscription id if missing and add it to the decoded payload.
                if mp["@id"] == "" {
                        mp["@id"] = tb.getUUID(w)
                }
                expandedMap = append(expandedMap, mp)
        }

        // Pretty print
        sub, _ = json.MarshalIndent(expanded, "", " ")
        DEBUG.Println("Subscription after adding ID:")
        DEBUG.Println(string(sub))
...
func (tb *ThinBroker) getStringInterfaceMap(w rest.ResponseWriter, r *rest.Request) map[string]interface{} {
...
func (tb *ThinBroker) getUUID(w rest.ResponseWriter) string {
        u1, err := uuid.NewV4()
        if err != nil {
                rest.Error(w, err.Error(), http.StatusInternalServerError)
                return ""
