...
class TestStringMethods(unittest.TestCase):
   ...
   # Test ld_generate without vertices
    ...
    def test_get_coordinate_withoutType(self):
        test_obj = metadata_converter(
            "", data.get_coordinate_input_forWithoutType)
        coordinate = test_obj.get_coordinate(
            data.get_coordinate_input_forWithoutType)
        self.assertEqual(coordinate, data.get_coordinate_output_forWithoutType)

    def test_get_coordinate_withoutValue(self):
        test_obj = metadata_converter(
            "", data.get_coordinate_input_forWithoutValue)
        coordinate = test_obj.get_coordinate(
            data.get_coordinate_input_forWithoutValue)
        self.assertEqual(
            coordinate, data.get_coordinate_output_forWithoutValue)

    def test_get_coordinate_withoutTypeValue(self):
        test_obj = metadata_converter(
            "", data.get_coordinate_input_forWithoutTypeValue)
        coordinate = test_obj.get_coordinate(
            data.get_coordinate_input_forWithoutTypeValue)
        self.assertEqual(
            coordinate, data.get_coordinate_output_forWithoutTypeValue)

    def test_domainMetaDataWithpolygon(self):
        test_obj = metadata_converter(
            "", data.domainMetaData_with_polygon_input)
        ngb_polygonMetaData = test_obj.polygon_type()
        self.assertEqual(ngb_polygonMetaData,
                         data.domainMetaData_with_withpolygon_output)

    def test_domainMetaDataWithpolygon_withoutVertices(self):
        test_obj = metadata_converter(
            "", data.domainMetaData_with_polygon_withoutVertices_input)
        ngb_polygonMetaData = test_obj.polygon_type()
        self.assertEqual(
            ngb_polygonMetaData, data.domainMetaData_with_polygon_withoutVertices_output)
