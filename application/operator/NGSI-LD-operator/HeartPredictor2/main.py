...
def notifyContext():
    ...
    return jsonify({'responseCode': 20000})
...
def handleNotify(contextObjs):
    for ctx in contextObjs:
        fogflow.handleEntity(ctx, publishResult, publishResultOnDesigner)
...
def publishResultOnDesigner(ctxObj):
    ...
    #ctxElement = object2Element(ctxObj)
    #ctxObj['id'] = "urn:ngsi-ld:Device.PredictionResult"
    #ctxObj['type'] = "HealthResult"

    print("======== published result On Designer =============")
    sys.stdout.flush()
    print(ctxObj)
    sys.stdout.flush()
