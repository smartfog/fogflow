...
def processInputStreamData(obj):
    ...
    if 'attributes' in obj:
        ...
        if 'url' in attributes:
            ...
            with lock:
                # fetch the captured image from a web camera
                result = faceCounting(cameraURL)
                # publish the generated result to the configured broker
...
def publishResult(result):
    ...
    resultCtxObj['attributes']['num'] = {
        'type': 'integer', 'value': result['facenum']}
    resultCtxObj['attributes']['totalbytes'] = {
        'type': 'integer', 'value': result['totalbytes']}
